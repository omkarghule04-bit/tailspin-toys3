...
Repository-wide standards are documented in [`.github/copilot-instructions.md`](.github/copilot-instructions.md) and the technology-specific files in [`.github/instructions/`](.github/instructions/).
Comments should explain intent and non-obvious decisions, not restate code. Exported functions in `db/` and `src/lib/` require TSDoc/JSDoc for their purpose, parameters, and return value, and reusable Astro components document their `Props` interface.
TypeScript formatting conventions (including four-space indentation, semicolons, single quotes, trailing commas, interface contracts, and `T[]` array syntax) are enforced by ESLint for the data and application source.
